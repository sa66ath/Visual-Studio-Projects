...
//------------------------------------------------------------------------------
class ParamRate : public VSTParamHasMinMax<float> {
public:
	ParamRate(const char* label, const char* name, float min, float max) : VSTParamHasMinMax<float>(label, name, min, max) {}
	void setVal(float value)
	{ 
		v = value;
		multiBand.osc.setFreq(readout());
	}
};
...
extern ParamRate		paramRate;

extern VSTParamHasMinMax<float>		paramAttack;
